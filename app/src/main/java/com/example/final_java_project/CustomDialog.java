...
public class CustomDialog extends Dialog {
    ...
    public CustomDialog(@NonNull Context context, int case_number) {
        ...
        if (case_number == 1) {
            ...
            txt_contents3.setText("앱의 화면 수 : 현재 화면, 회원가입, 거주지 검색, 다이얼러그(2종류), 로그인(여행객 로그인, 가이드 로그인), 여행객 메인 화면, 여행객 정보수정 화면, 여행객 채팅 화면, 가이드 메인 화면, 가이드 정보 수정 화면, 가이드 채팅 화면 .\n");
            txt_contents4.setText("기능 : 회원가입(가이드, 여행객), 로그인(가이드, 여행객), 거주지 검색, 채팅, 화면정보 설명 다이얼러그, 계정 정보 수정, 가이드 별점 평가, 로그아웃, 깃허브 레퍼지토리 버튼\n");
            txt_contents5.setText("");
            txt_contents6.setText("");
        }
